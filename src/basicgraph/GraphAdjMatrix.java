...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		int [][] originalAdjMatrix = new int [getNumVertices()][getNumVertices()];
		int[][] newAdjMatrix = new int[getNumVertices()*2][getNumVertices()*2];
		originalAdjMatrix = adjMatrix;
		int row = adjMatrix.length;
		int column = adjMatrix.length;
		int sum;	
		for (int i = 0; i < row; i ++) {			
			for (int j = 0; j < column ; j++) {
				...
				newAdjMatrix[i][j]= sum;
			}			
		}
		adjMatrix=newAdjMatrix;
		List <Integer> twoHopList = new ArrayList <Integer> ( );
		List <Integer> finalTwoHopList = new ArrayList <Integer>();
		twoHopList.addAll(getNeighbors(v));
		finalTwoHopList.addAll(twoHopList);
		adjMatrix=originalAdjMatrix;
		Collections.sort(finalTwoHopList);
		Collections.reverse(finalTwoHopList);
		return finalTwoHopList;
	}
